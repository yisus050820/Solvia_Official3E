...
const PerfilUsuario = () => {
  ...
  const validateProfile = (editInfo, userInfo, password) => {
    const validationErrors = {};
    const age = editInfo.birthdate ? calculateAge(editInfo.birthdate) : 0;

    if (age > 100) {
      validationErrors.birthdate = "Ingresa una fecha de nacimiento válida.";
    } else if (userInfo.role === "beneficiary" && age < 9) {
      validationErrors.birthdate = "Debes tener al menos 9 años.";
    } else if (userInfo.role !== "beneficiary" && age < 18) {
      validationErrors.birthdate = "Debes tener al menos 18 años.";
    }

    if (!editInfo.name || editInfo.name.trim().length < 3) {
      validationErrors.nombre = "El nombre es obligatorio y debe tener al menos 3 caracteres.";
    }

    if (!editInfo.email || !/\S+@\S+\.\S+/.test(editInfo.email)) {
  ...
  const handleSave = async () => {
    const validationErrors = validateProfile(editInfo, userInfo, password);
          ...
          >
            ...
            >
              ...
              <div className="flex justify-center items-center">
                ...
                >
                  <FaCheck size={50} style={{ color: '#ffffff' }} />
                </motion.div>
              </div>
            </motion.div>
          </motion.div>
