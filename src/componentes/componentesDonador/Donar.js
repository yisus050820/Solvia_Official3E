...
const Donar = () => {
  ...
  const validateForm = () => {
    ...
    if (!nombre || !direccion || !ciudad || !codigoPostal || !pais || !amount || !metodoPago) {
  ...
  // Función para generar el PDF de la donación con imagen en el encabezado y firma simulada con el texto "SOLVIA"
  const generarFacturaPDF = () => {
    ...
    doc.text(`${nombre}`, 20, 78);
    doc.text(`${direccion}`, 20, 84);
    doc.text(`${ciudad}, ${codigoPostal}`, 20, 90);
    ...
    // Valores de la tabla
    doc.setFont('helvetica', 'normal');
    doc.text('Donación para Solvia', 25, 116);
    doc.text('1', 110, 116);
    doc.text(`${Number(amount).toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}`, 160, 116);
    ...
    // Firma simulada con el texto "SOLVIA"
    doc.setFont('helvetica', 'bold');
    doc.text('SOLVIA ONG', 20, 160); // Texto SOLVIA encima de la línea de firma
    doc.setDrawColor(0);
    doc.setLineWidth(0.5);
    doc.line(20, 165, 80, 165); // Línea de firma
    doc.setFont('helvetica', 'italic');
    doc.text('Firma de solvia', 20, 175); // Firma simulada debajo de la línea

    // Mensaje de agradecimiento
    doc.setFontSize(16);
    doc.setTextColor(255, 165, 0);
    doc.text('¡Gracias!', 160, 180);
